...
{
    public static class CreateDecalLight
    {
        [MenuItem("GameObject/Light/CRP Decal Light", priority = CoreUtils.Priorities.gameObjectMenuPriority)]
        public static void CreateDecal(MenuCommand menuCommand)
        {
            var go = CoreEditorUtils.CreateGameObject("Decal Light", menuCommand.context);
            go.AddComponent<DecalLight>();
            go.transform.RotateAround(go.transform.position, go.transform.right, 90);
        }
    }
    
    [CustomEditor(typeof(DecalLight))]
    [CanEditMultipleObjects]
    public class DecalLightEditor : UnityEditor.Editor
    {
        ...
        static List<DecalLightEditor> s_Instances = new List<DecalLightEditor>();

        static DecalLightEditor FindEditorFromSelection()
        {
            GameObject[] selection = Selection.gameObjects;
            DecalLight[] selectionTargets = Selection.GetFiltered<DecalLight>(SelectionMode.Unfiltered);

            foreach (DecalLightEditor editor in s_Instances)
            {
                if (selectionTargets.Length != editor.targets.Length)
                    continue;
                bool allOk = true;
                foreach (DecalLight selectionTarget in selectionTargets)
                {
        ...
        {
            DecalLight decalLight = target as DecalLight;

            return new Bounds(decalLight.transform.position, boxHandle.size);
        }
        ...
        public void UpdateMaterialEditor()
        {
            ...
            {
                DecalLight decalLight = (targets[index] as DecalLight);
                if ((decalLight != null) && (decalLight.material != null))
                    validMaterialsCount++;
            }
            ...
            for (int index = 0; index < targets.Length; ++index)
            {
                ...
                if ((decalLight != null) && (decalLight.material != null))
                    materials[validMaterialsCount++] = (targets[index] as DecalLight).material;
            }
        ...
        void DrawBoxTransformationHandles(DecalLight decalLight)
        {
            Vector3 scale = decalLight.effectiveScale;
            using (new Handles.DrawingScope(fullColor, Matrix4x4.TRS(decalLight.transform.position, decalLight.transform.rotation, scale)))
            {
                Vector3 centerStart = decalLight.pivot;
                boxHandle.center = centerStart;
                boxHandle.size = decalLight.size;
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    Undo.RecordObject(decalLight, "Decal Projector Change");
                    ...
                    decalLight.size = new Vector3(
                        xChangeIsValid ? boxHandle.size.x : decalLight.size.x,
                        yChangeIsValid ? boxHandle.size.y : decalLight.size.y,
                        zChangeIsValid ? boxHandle.size.z : decalLight.size.z);
                    decalLight.pivot = new Vector3(
                        xChangeIsValid ? boxHandle.center.x : decalLight.pivot.x,
                        yChangeIsValid ? boxHandle.center.y : decalLight.pivot.y,
                        zChangeIsValid ? boxHandle.center.z : decalLight.pivot.z);
                    ...
                    if (EditMode.editMode == k_EditShapePreservingUV)
                    {
                        ...
                        // Preserve serialized state for axes with scale 0.
                        Vector2 uvScale = decalLight.uvScale;
                        Vector2 uvBias = decalLight.uvBias;
                        if (xChangeIsValid)
                        {
                        ...
                        decalLight.uvScale = uvScale;
                        decalLight.uvBias = uvBias;
                    }

                    if (PrefabUtility.IsPartOfNonAssetPrefabInstance(decalLight))
                    {
                        PrefabUtility.RecordPrefabInstancePropertyModifications(decalLight);
                    }
                }
            }
        }

        void DrawPivotHandles(DecalLight decalLight)
        {
            Vector3 scale = decalLight.effectiveScale;
            Vector3 scaledPivot = Vector3.Scale(decalLight.pivot, scale);
            Vector3 scaledSize = Vector3.Scale(decalLight.size, scale);

            using (new Handles.DrawingScope(fullColor, Matrix4x4.TRS(Vector3.zero, decalLight.transform.rotation, Vector3.one)))
            {
                EditorGUI.BeginChangeCheck();
                Vector3 newPosition = ProjectedTransform.DrawHandles(decalLight.transform.position, .5f * scaledSize.z - scaledPivot.z, decalLight.transform.rotation);
                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObjects(new UnityEngine.Object[] { decalLight, decalLight.transform }, "Decal Projector Change");

                    scaledPivot += Quaternion.Inverse(decalLight.transform.rotation) * (decalLight.transform.position - newPosition);
                    decalLight.pivot = new Vector3(
                        scale.x != 0f ? scaledPivot.x / scale.x : decalLight.pivot.x,
                        scale.y != 0f ? scaledPivot.y / scale.y : decalLight.pivot.y,
                        scale.z != 0f ? scaledPivot.z / scale.z : decalLight.pivot.z);
                    decalLight.transform.position = newPosition;
        ...
        void DrawUVHandles(DecalLight decalLight)
        {
            Vector3 scale = decalLight.effectiveScale;
            Vector3 scaledPivot = Vector3.Scale(decalLight.pivot, scale);
            Vector3 scaledSize = Vector3.Scale(decalLight.size, scale);

            using (new Handles.DrawingScope(Matrix4x4.TRS(decalLight.transform.position + decalLight.transform.rotation * (scaledPivot - .5f * scaledSize), decalLight.transform.rotation, scale)))
            {
                Vector2 uvScale = decalLight.uvScale;
                Vector2 uvBias = decalLight.uvBias;

                Vector2 uvSize = new Vector2(
                    (uvScale.x > k_Limit || uvScale.x < -k_Limit) ? 0f : decalLight.size.x / uvScale.x,
                    (uvScale.y > k_Limit || uvScale.y < -k_Limit) ? 0f : decalLight.size.y / uvScale.y
                );
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    for (int channel = 0; channel < 2; channel++)
                    {
                        ...
                        if (scaledSize[channel] != 0f)
                        {
                            ...
                            float decalSize = decalLight.size[channel];
        ...
        void DrawHandles()
        {
            ...
            if (EditMode.editMode == k_EditShapePreservingUV || EditMode.editMode == k_EditShapeWithoutPreservingUV)
                DrawBoxTransformationHandles(decalLight);
            else if (EditMode.editMode == k_EditUVAndPivot)
            {
                DrawPivotHandles(decalLight);
                DrawUVHandles(decalLight);
            }
        }

        [DrawGizmo(GizmoType.Selected | GizmoType.Active)]
        static void DrawGizmosSelected(DecalLight decalLight, GizmoType gizmoType)
        {
            ...
            using (new Handles.DrawingScope(fullColor, Matrix4x4.TRS(decalLight.transform.position, decalLight.transform.rotation, Vector3.one)))
            {
                Vector3 scale = decalLight.effectiveScale;
                Vector3 scaledPivot = Vector3.Scale(decalLight.pivot, scale);
                Vector3 scaledSize = Vector3.Scale(decalLight.size, scale);
                ...
                using (new Handles.DrawingScope(Matrix4x4.TRS(decalLight.transform.position + decalLight.transform.rotation * new Vector3(scaledPivot.x, scaledPivot.y, scaledPivot.z - .5f * scaledSize.z), decalLight.transform.rotation, Vector3.one)))
                {
                    Vector2 UVSize = new Vector2(
                        (decalLight.uvScale.x > k_Limit || decalLight.uvScale.x < -k_Limit) ? 0f : scaledSize.x / decalLight.uvScale.x,
                        (decalLight.uvScale.y > k_Limit || decalLight.uvScale.y < -k_Limit) ? 0f : scaledSize.y / decalLight.uvScale.y
                    );
                    Vector2 UVCenter = UVSize * .5f - new Vector2(decalLight.uvBias.x * UVSize.x, decalLight.uvBias.y * UVSize.y) - (Vector2)scaledSize * .5f;
        ...
        static Func<Bounds> GetBoundsGetter(DecalLight decalLight)
        {
            return () =>
            {
                var bounds = new Bounds();
                var decalTransform = decalLight.transform;
        ...
        Dictionary<DecalLight, Vector3> ratioSizePivotPositionSaved = null;
        ...
        {
            void UpdateSizeOfOneTarget(DecalLight currentTarget)
            {
                ...
                {
                    ratioSizePivotPositionSaved = new Dictionary<DecalLight, Vector3>();
                    foreach (DecalLight projector in targets)
            ...
            // update each target
            foreach (DecalLight decalLight in targets)
            {
                UpdateSizeOfOneTarget(decalLight);
                ...
                if (PrefabUtility.IsPartOfPrefabAsset(decalLight))
                    EditorUtility.SetDirty(decalLight);
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                EditMode.DoInspectorToolbar(k_EditVolumeModes, editVolumeLabels, GetBoundsGetter(target as DecalLight), this);
                GUILayout.FlexibleSpace();
                ...
                {
                    var decalLight = target as DecalLight;

                    float combinedScale = decalLight.transform.lossyScale.x * decalLight.transform.lossyScale.y * decalLight.transform.lossyScale.z;
                    negativeScale |= combinedScale < 0 && decalLight.scaleMode == DecalScaleMode.InheritFromHierarchy;
                }
                if (negativeScale)
                {
                ...
                {
                    var decalLight = target as DecalLight;
                    var mat = decalLight.material;

                    isDefaultMaterial |= decalLight.material == DecalLight.defaultMaterial;
                    isValidDecalMaterial &= decalLight.IsValid();
                }
                ...
                if (m_MaterialEditor && !isValidDecalMaterial)
                {
                    ...
                    {
                        m_MaterialProperty.objectReferenceValue = DecalLight.defaultMaterial;
                ...
                foreach (var decalLight in targets)
                {
                    var mat = (decalLight as DecalLight).material;
                    if (mat == null)
                        continue;
            ...
            if (layerMaskHasMultipleValues || layerMask != (target as Component).gameObject.layer)
            {
                foreach (var decalLight in targets)
                {
                    (decalLight as DecalLight).OnValidate();
                }
            }
...
//
        ...
        [Shortcut("CRP/DecalLight: Handle changing size stretching UV", typeof(SceneView), KeyCode.Keypad1, ShortcutModifiers.Action)]
        static void EnterEditModeWithoutPreservingUV(ShortcutArguments args)
        {
            ...
            DecalLight activeDecalProjector = Selection.activeGameObject?.GetComponent<DecalLight>();
        ...
        [Shortcut("CRP/DecalLight: Handle changing size cropping UV", typeof(SceneView), KeyCode.Keypad2, ShortcutModifiers.Action)]
        ...
        [Shortcut("CRP/DecalLight: Handle changing pivot position and UVs", typeof(SceneView), KeyCode.Keypad3, ShortcutModifiers.Action)]
        ...
        [Shortcut("CRP/DecalLight: Handle swap between cropping and stretching UV", typeof(SceneView), KeyCode.Keypad4, ShortcutModifiers.Action)]
        static void SwappingEditUVMode(ShortcutArguments args)
        {
            //If editor is not there, then the selected GameObject does not contains a DecalLight
            DecalLight activeDecalProjector = Selection.activeGameObject?.GetComponent<DecalLight>();
            if (activeDecalProjector == null || activeDecalProjector.Equals(null))
                return;
        ...
        [Shortcut("CRP/DecalLight: Stop Editing", typeof(SceneView), KeyCode.Keypad0, ShortcutModifiers.Action)]
